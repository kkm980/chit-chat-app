...
socketServer.on('connection', skt=>{
    ...
    skt.emit('msg', msgSatchet("abc","welcome"));

    // to all clients except the client who is triggering the event(tax payers money)--->
    skt.broadcast.emit("msg", msgSatchet("abc", "someone joined"));
    ...
    skt.on('disconnect', ()=>{
        socketServer.emit('msg', msgSatchet("abc", "someone left"));
    })

    // receive new message 
     skt.on('newMsg', chat=>{
        // console.log(chat); 
        socketServer.emit('msg', msgSatchet("user", chat));
     })
})
